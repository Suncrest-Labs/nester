...
class AllocationItem(BaseModel):
    ...
    weight: float = Field(..., ge=0, le=100, description="Allocation weight as percentage")
...
class PortfolioBreakdown(BaseModel):
    ...
    total_value_usdc: float = Field(..., ge=0, description="Total portfolio value in USDC")
    yield_30d_usdc: float = Field(..., ge=0, description="Expected yield over 30 days in USDC")
    ...
    top_recommendation: str = Field(..., description="Primary recommendation for the user")
    rebalance_suggested: bool = Field(..., description="Whether rebalancing is recommended")


class PortfolioAnalysisResponse(BaseModel):
    """Complete response from portfolio analysis endpoint."""

    analysis: PortfolioBreakdown = Field(..., description="Structured portfolio breakdown")
